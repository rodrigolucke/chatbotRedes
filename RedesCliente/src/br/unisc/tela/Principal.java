...
public class Principal extends javax.swing.JFrame {
	...
	/**
	 * comando a ser criado e enviado para servidor.<br>
	 * utilizado tambem para gravar respostas do servidor
	 */
	...
	public Principal() {
		...
		tf_Porta.setText("1234");
	}

	/**
	 * thread de inicializacao do form grafico
	 * 
	 * @author jLucke
	 *
	 */
	...
	private void bt_enviarActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_bt_enviarActionPerformed
		ChatBot envio = new ChatBot();
		envio.setNome(comando.getNome());
		envio.setTexto(tf_mensagem.getText());
		envio.setAcao(Acao.ENVIAR);
		this.ta_chat.append(this.comando.getNome() + ": " + tf_mensagem.getText() + "\n");

		try {
			this.cliente.enviaMensagem(envio);
		} catch (IOException ex) {
			Logger.getLogger(Principal.class.getName()).log(Level.SEVERE, null, ex);
		}
		this.tf_mensagem.setText("");
	...
	private void bt_conectarActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_bt_conectarActionPerformed
		if (tf_Ip.getText().equals("") || tf_Porta.getText().equals("")) {
			...
			try {
				...
				this.comando.setTexto("Nova conexão requisitada: ");
				this.comando.setNome("");
				this.tf_Ip.setEnabled(false);
				this.tf_Porta.setEnabled(false);
				...
				try {
					...
					this.ta_chat.append("Erro ao conectar em " + tf_Ip.getText() + " : " + tf_Porta.getText() + "\n");
					this.tf_Ip.setEnabled(true);
					this.tf_Porta.setEnabled(true);
				}
			} catch (IOException ex) {
	...
	private void receber(ChatBot comando) {
		this.ta_chat.append(comando.getNome() + comando.getTexto() + "\n");
	}
